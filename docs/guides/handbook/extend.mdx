...
Here, by using the `-st-extends` property on a local class, we are scoping these customization to the sub-tree of our component stylesheet.

### Apply the extending class

In order to be able to customize a `Player` component instance using our local stylesheet, we will need to apply the extending local class to the instance of the component.
...
We can now use the custom pseudo-element syntax to safetly target the `avatar` and `username` parts of the `Player` component.
...
:::note You've reached the end of the handbook, so far
